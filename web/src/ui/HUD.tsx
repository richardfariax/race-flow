...
const styles: Record<string, React.CSSProperties> = {
  root: { position: 'fixed', inset: 0, pointerEvents: 'none', color: '#fff', userSelect: 'none' },
  logo: {
    position: 'absolute',
    top: 16,
    left: 20,
  ...
  speed: { position: 'absolute', bottom: 24, right: 28, textAlign: 'right', textShadow: '2px 2px 0 #1a1a2e' },
  ...
  return (
    <div style={styles.root}>
      ...
      {online && phase === 'countdown' && (
        <div style={styles.countdown}>{Math.max(1, Math.ceil(countdownMs / 1000))}</div>
      )}

      {online && (phase === 'racing' || phase === 'finished') && mode === 'circuit' && me && (
        <div style={styles.race}>
          <div style={{ fontSize: 26 }}>
            {position}º<span style={{ opacity: 0.7 }}>/{standings.length}</span>
          </div>
          <div style={{ fontSize: 18 }}>
            Volta {Math.min(me.lap, totalLaps)}/{totalLaps}
          </div>
          <div style={{ fontSize: 14, opacity: 0.85 }}>⏱ {fmtClock(raceTimeMs)}</div>
          {me.bestLapMs > 0 && (
            <div style={{ fontSize: 12, opacity: 0.75 }}>melhor volta {fmtClock(me.bestLapMs)}</div>
          )}
        </div>
      )}

      {online && (phase === 'racing' || phase === 'finished') && mode === 'drift' && me && (
        <div style={styles.race}>
          <div style={{ fontSize: 30, color: '#ffd166' }}>
            {me.driftScore.toLocaleString('pt-BR')} pts
          </div>
          {me.driftCombo > 1 && (
            <div style={{ fontSize: 20, color: '#f15bb5' }}>combo ×{me.driftCombo.toFixed(1)}</div>
          )}
